...
(async () => {
  ...
  try {
    ...
    // 2. Topic Selection & Marketing Banner
    console.log('[TEST] Verifying Topic Selection & Marketing Banner...');
    const hasBanner = await page.evaluate(() => {
      return document.body.innerText.includes('Detailed, customized explanations for every question');
    });
    if (!hasBanner) throw new Error("Marketing banner is missing!");

    // 3. Start a Quiz
    console.log('[TEST] Starting General Medicine Quiz...');
    await page.evaluate(() => {
      const topic = Array.from(document.querySelectorAll('div')).find(d => d.innerText.includes('General Medicine & Pathology') && !d.innerText.includes('Topics'));
    ...
    const paywallOpen = await page.evaluate(() => {
      return document.body.innerText.includes('.99/mo') && document.body.innerText.includes('.99');
    });
    if (!paywallOpen) throw new Error("Paywall modal failed to open or is missing strikethrough pricing.");

    // Close paywall
    await page.evaluate(() => {
      const closeBtn = Array.from(document.querySelectorAll('button')).find(b => b.innerText === '✕' || b.innerHTML.includes('XCircle'));
      if (closeBtn) closeBtn.click();
      // fallback if it's lucide icon
